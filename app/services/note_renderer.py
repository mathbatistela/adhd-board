...
class NoteRendererService:
    ...
    def build_html(
        ...
        return (
            template_html.replace("{{ category_icon }}", escape(category_icon))
            .replace("{{ category_label }}", escape(category_label))
            .replace("{{ category_icon_svg }}", category_icon_svg)
    ...
    def render_note(
        ...
        """
        metadata = self._get_category_metadata(category)
        html = self.build_html(
            template_html=template_html,
            category_icon=metadata.emoji,
            category_label=metadata.label,
            category_icon_svg=metadata.svg,
            ticket_id=ticket_id,
            text=text,
            date=date,
