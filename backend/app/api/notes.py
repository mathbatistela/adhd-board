...
class NoteDetail(MethodView):
    ...
    @notes_bp.arguments(NoteUpdateSchema)
    @notes_bp.response(200, NoteResponseSchema)
    def patch(self, data, note_id):
        """Update a note."""
        note_service = current_app.note_service
        try:
            ...
            current_app.logger.error(f"Failed to update note {note_id}: {exc}")
            abort(500, message="Failed to update note")

    @notes_bp.response(204)
